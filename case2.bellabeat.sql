...
SELECT
 ...
 COUNT(table_name)
FROM
 `case-2-343323.bellabeat.INFORMATION_SCHEMA.COLUMNS`
GROUP BY
 1;
 -- Id was a common column, let's make sure that it is in every table we have
SELECT
 table_name,
 SUM(CASE
...
ORDER BY
 ...
 -- If the column types were detected properly prior to upload this table should be empty
SELECT
 table_name,
 SUM(CASE
...
-- if we want to do an analysis based upon daily data, this could help us to find tables that might be at the day level
SELECT
 DISTINCT table_name
FROM
 `case-2-343323.bellabeat.INFORMATION_SCHEMA.COLUMNS`
...
FROM
 `case-2-343323.bellabeat.daily_activity` A
LEFT JOIN
 `case-2-343323.bellabeat.daily_calories` C
ON
 A.Id = C.Id
 AND A.ActivityDate=C.ActivityDay
 AND A.Calories = C.Calories
LEFT JOIN
 `case-2-343323.bellabeat.daily_intensities` I
ON
 A.Id = I.Id
...
LEFT JOIN
 `case-2-343323.bellabeat.daily_steps` S
ON
 A.Id = S.Id
 AND A.ActivityDate=S.ActivityDay
LEFT JOIN
 `case-2-343323.bellabeat.sleep_day` Sl
ON
 A.Id = Sl.Id
 AND A.ActivityDate=Sl.SleepDay;
 
 -- If we are considering sleep related products as a possibility, let's take a moment to see if/ how people nap during the day
...
FROM (
 ...
 FROM
   `case-2-343323.bellabeat.minute_sleep`
 WHERE
   value=1
 GROUP BY
   1,
...
ORDER BY
 ...
 -- If we would like to do an analysis based upon the time of day and day of the week
...
WITH
 ...
 FROM
   `case-2-343323.bellabeat.hourly_intensities`
 GROUP BY
   1,
   2,
   3,
